...
/** Minimal KEY=value parser — no dependency quirks, no shell semantics. */
function readEnvFile(path: string): Record<string, string> {
  ...
  try {
    ...
    return out // file absent — not an error, every source here is optional
  }
 ...
 * process.env, so read the files directly. Real environment variables win when
 * valid, then `.env`, then `.worktree/ports.env`.
 *
 * `.worktree/ports.env` is generated by scripts/portlock.mjs and carries the
 * DATABASE_URL of this checkout's throwaway docker-compose stack. It comes last
 * on purpose: pointing eng-ops at your own database is the normal case, so a
 * DATABASE_URL you set anywhere else has to win over the generated default.
 */
function loadRawEnv(): {
  DATABASE_URL: string
